		...
		foreach ( $transactions_queue as $transaction ) {
			switch ( $transaction['type'] ) {
				...
				case 'single':
					...
					$anticipation_payment = ( new Anticipation_Meta() )->get_anticipation_payment_option();
					if ( $anticipation_payment && isset( $anticipation_payment['allowAnticipation'] ) ) {
						( new Anticipation_Interest_Handler( $wc_order ) )->request_anticipation( $json );
					}
