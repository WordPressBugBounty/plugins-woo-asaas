...
* Tweak: Added compatibility with WooCommerce High-Performance Order Storage (HPOS).
...
* Dev: Updated authToken for webhooks to 32 characters.
* Tweak: Compatibilized webhook queue reactivation component with exponential penalty model.
...
* Tweak - Set PIX copy/paste enabled by default
* Fix - Windows compability
* Fix - Upgrade with legacy split data

= 2.7.0 =

* Feature - Split wallet administration screen
* Fix - Webhook health check report
...
* Dev - Add filter `woocommerce_asaas_should_process_webhook` to allow abort webhook processing
* Feature - Warn about impact of split settings in new major update
...
* Fix - API key and Webhook settings
* Fix - Webhook wrong message about billing type 

= 2.6.2 =

* Fix - Deprecated warning for latest PHP versions
* Fix - Webhook status report
* Fix - Webhook event handling

= 2.6.1 =

* Fix - Expired PIX removal action via cron when HPOS is enabled
* Fix - Checkout when a new account is created
* Fix - Duplicate Asaas customer creation at checkout
* Fix - Pix and ticket checkout error in newer versions of WooCommerce

= 2.6.0 =

* Feature - Allow enabling credit advance on credit card settings page
* Tweak - Add front-end source code to plugin package
* Fix - Remove duplicated information on thank you page
* Fix - Definition of next payment date on Subscriptions
* Fix - Security issues

= 2.5.2 =

* Fix - Change behavior on subscription Pending Cancellation status
* Fix - Subscriptions coupons

= 2.5.1 =

* Fix - Matches the access token validation in the webhook configuration
* Fix - Incompatibility issue with MercadoPago

= 2.5.0 =

* Feature - Automate webhooks configuration process
* Fix - Security issues
